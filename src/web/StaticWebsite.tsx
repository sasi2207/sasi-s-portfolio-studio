...
const pricingPlans = [
  ...
  },
  {
    ...
    features: [
      "Multi-page structured architecture (Up to 10 Pages)",
      ...
      <div className="bg-black text-white min-h-screen selection:bg-orange-500 selection:text-white bg-grid-box-full relative">
        ...
        {selectedPlan && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/85 backdrop-blur-md animate-fade-in">
            {/* Modal Box Styled in Landscape Aspect Format */}
            <div className="bg-zinc-900 border border-orange-500/40 rounded-3xl max-w-3xl w-full p-8 md:p-10 relative shadow-2xl flex flex-col md:flex-row gap-8 items-stretch">
              
              {/* Close Button */}
              <button
                onClick={() => setSelectedPlan(null)}
                className="absolute top-5 right-5 text-zinc-400 hover:text-white bg-zinc-800 p-2 rounded-full transition cursor-pointer z-10"
              >
                <X size={20} />
              </button>

              {/* Left Column in Landscape Modal: Title, Pricing & Overview */}
              <div className="flex-1 flex flex-col justify-between border-b md:border-b-0 md:border-r border-zinc-800 pb-6 md:pb-0 md:pr-8">
                <div>
                  <span className="text-xs uppercase tracking-widest text-orange-400 font-bold bg-orange-500/10 border border-orange-500/30 px-3 py-1 rounded-full inline-block mb-3">
                    Selected Package
                  </span>
                  <h3 className="text-3xl font-bold text-white mb-2">{selectedPlan.title}</h3>
                  <div className="text-3xl font-extrabold text-orange-400 mb-4">{selectedPlan.price}</div>
                  <p className="text-zinc-300 text-sm mb-6 leading-relaxed">{selectedPlan.description}</p>
                </div>
                ...
                <div className="grid grid-cols-2 gap-3 bg-zinc-950 p-4 rounded-2xl border border-zinc-800">
                  ...
                  <div>
                    ...
                    <span className="text-white text-xs font-medium">{selectedPlan.idealFor}</span>
                  </div>
                </div>
              </div>
              ...
              <div className="flex-1 flex flex-col justify-between">
                <div>
                  <h4 className="text-xs uppercase tracking-wider text-zinc-400 font-bold mb-4">
                ...
                {/* Action Buttons inside Landscape Modal */}
                <div className="flex flex-col sm:flex-row gap-3 pt-4 border-t border-zinc-800">
                  <button
                    onClick={() => handleWhatsAppClick(selectedPlan.title)}
                    className="flex-1 bg-emerald-600 hover:bg-emerald-500 text-white font-bold py-3.5 px-5 rounded-xl transition flex items-center justify-center gap-2 shadow-lg shadow-emerald-600/20 cursor-pointer text-sm"
                  >
                    <MessageCircle size={18} /> Confirm via WhatsApp
                  </button>
                  <button
                    onClick={() => setSelectedPlan(null)}
                    className="bg-zinc-800 hover:bg-zinc-700 text-zinc-300 font-semibold py-3.5 px-5 rounded-xl transition cursor-pointer text-sm"
                  >
                    Close
                  </button>
                </div>
